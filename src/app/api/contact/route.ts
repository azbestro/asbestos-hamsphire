...
export async function POST(request: Request) {
  try {
    ...
    const FROM_EMAIL = "noreply@asbestosserviceshampshire.uk";
    ...
    // Email to business (critical — must succeed)
    ...
    if (businessResult.error) {
      ...
      return NextResponse.json(
        { error: `Email delivery failed: ${businessResult.error.message}. Please call us on 07424 521865.` },
        { status: 500 }
      );
    }

    // Confirmation email to client (non-critical — don't fail the whole request)
    try {
      ...
      // Client confirmation failed (likely domain not verified yet) — not critical
      console.warn("Client confirmation email failed:", confirmError);
    }
    ...
    return NextResponse.json(
      { error: `Something went wrong: ${message}. Please call us on 07424 521865.` },
      { status: 500 }
    );
  }
}
